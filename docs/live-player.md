...
4. 点击“加载学期”，从返回的列表中手动选择学期，按课程名、教师、学院或课程代码搜索，勾选后点击“保存关注课程”。同名课程可通过教师、学院和课程代码区分；最多关注 200 门。备用的“手动输入课程链接或 ID”仍可粘贴详情链接。
5. 点击“刷新直播”；已关注课程会一直显示，卡片标注“正在直播”“暂无直播”或“状态未知”。只有“正在直播”的课程可以点击“打开直播”。播放器支持平台提供的教师画面、学生画面及纯音频来源。

学期从官方最近课程页面读取，显示的是近期目录实际出现的学期，不是全部历史学期。首次搜索需要读取该学期全部分页，后续搜索使用五分钟内存缓存。选择结果会连同学期和课程公开元数据保存在扩展本地存储中，扩展重启后仍会显示在“已关注课程”列表。目录课程可见不代表此刻正在直播。
...
按提示输入学号和统一身份认证密码，再从官方返回的学期列表中输入编号，按课程名、教师或学院搜索，输入结果编号，可多次搜索添加；最后回车完成选择。密码不回显、不由启动器写入文件。课程 ID、所选学期和选中课程的公开元数据会保存到 `course-selection.json`，下次直接复用；运行 `python -m live_player --interactive --select-courses` 可重新选择。也可预先设置 `StuId`、`UISPsw`、`COURSE_IDS` 后启动。macOS 优先使用 Chrome，Windows 优先使用 Edge。
